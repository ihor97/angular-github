...
@NgModule({
    // якщо ми підключаємо активний модуль з сервісами в AppModule то сервіси так само підключаються якби сервіси були б в AppModule
    // якщо ми юхаємо lazy load то юзається child injector навідміну від root injector
    // в lazy load коли підгружається модуль створюється інстанс сервіса імеено для нього 
    // можна так робити коли ти хочеш мати окремий інстанс сервіса для даного модуля
